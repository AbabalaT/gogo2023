...
def mission_step_callback(event):
    ...
    if current_step == 0:
        ...
        if abs(current_x - f2x) < 0.25:
            if abs(current_y - f2y) < 0.25:
                current_step = 102
    elif current_step == 102:
        if abs(current_x - f3x) < 0.25:
            if abs(current_y - f3y) < 0.25:
                current_step = 14
...
def mission_act_callback(event):
    ...
    if current_step == 0:
        ...
        pose.pose.position.x = f2x
        pose.pose.position.y = f2y
        pose.pose.position.z = passing_height
        nav_teb = True
    elif current_step == 102:
        pose.pose.position.x = f3x
        pose.pose.position.y = f3y
        pose.pose.position.z = passing_height
        nav_teb = True
    elif current_step == 14:
